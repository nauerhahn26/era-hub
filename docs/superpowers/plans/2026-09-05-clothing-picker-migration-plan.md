...
5. [ ] **T1.5 `tests/clothing-variety.test.mjs` — the ported gate + `tests/synthetic-wardrobe.mjs`**
   ...
   - **Gate:** `rae-flow:reviewing` — **Scope:** `clothing-rank.js` + both pure suites against spec §3.1 rule 4, §3.2, §3.4, §4 (loved/staples), §8 first two bullets, and `outfit_set.py:61-73, 242-557` for parity. **Pass criteria:** every constant and order-of-operations item in A3-I3 is present and cited; deviations are exactly A4 items 4 and I5; no fs/Date/random in the module. Then **Phase 1 retrospective** in the workpad.
