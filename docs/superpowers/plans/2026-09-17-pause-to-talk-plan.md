...
**Gate 4 (after 3½, 9/17, 09:38 UTC).** `ERA_WT_SUFFIX=--wt-pause-to-talk bash tools/era-gate.sh` → `== era-gate: 99 passed, 0 failed ==` (log `scratchpad/gate4.log`). Five stamps, none dirty: hub 8a06263 (7add8ef), era-core 3626b30 (fac7336), era-board 88413cb (43b9968), era-making-words bc8e2f6 (8ba21be), era-pencil d4f1870 (da24e83). They lapse ~11:38 UTC; T10 lands inside that window.

**T9 step 3 — the tablet, for real (9/17, 09:40–09:52 UTC, session LOCKED).** The plan's `build-dist.sh --dry-run` does not exist (that flag is release.sh's); `build-dist.sh v0.33.6 <dist> --patch` off the gated tree → build **20260917.0938**, TREE 8a06263, era-scan PASS (warnings only), unpublished. `push-device tablet` 20260916.1730 → 20260917.0938; GET `/settings` on the tablet: `pauseGoes:"tdsnap"`, dwell 1000 → doors 2000 / tiles 1000 (`holds ["1000","2000"]` on the live page); installed `start-hub.bat:44` carries the `/kiosk/resume` curl. The Songs tile stood in for TD Snap: a one-shot schtasks task (`UserId Auerh`, `InteractiveToken`, registered by the admin ssh user, no password) ran exactly the tile's launcher with `ERA_QA_CDP=9222`; the page was driven from Linux over `ssh -L` + `connectOverCDP` (`scratchpad/t9-tablet-drive.mjs`); media muted at the element (02:40 at the house). Deleted afterwards: task, `pause-qa\` dir, kiosk 16308.
- Round trip 1 (DevTools DETACHED between the hold and the tap — the honest run): tile → hub `[kiosk] resume /board?recipe=songs: nothing is paused` (409) → kiosk 16308 launched. 💬 → `youre-welcome` paused at t=1.799, hub `[kiosk] paused /board?recipe=songs — handing the screen to her talker`, `stepaside.log: minimized 16308 rc True`. Tile again → launcher exit 0, **no relaunch**, hub `[kiosk] resumed /board?recipe=songs`, `foreground.log: settle 16308 pos True fg False / settle min True / settle restore True fg False / settle front False / found`. Page: `visible`@93733 → `Dwell.suppress(600)` → `onResume` → same song playing, t 2.385 → 3.41 → 4.44. **The design's return signal works on Windows: SetWindowPos on the parked window raised `visibilitychange`, the bar suppressed arming 600 ms, the song picked up where she left it.** Then `hidden` 27 ms later (the settle's min step, taken because `fg False`) with no `visible` after the restore until DevTools re-attached.
- Round trip 2 (DevTools ATTACHED throughout, 75 s): hub side identical (`paused`, `minimized rc True`, `resumed`, same settle lines at 09:47:51) but the page saw NO `visibilitychange` at all — not on the minimize, not on the restore — so `paused` stayed true. Every visibility event in the log lines up with an attach/detach of the DevTools client, not with a window move. So attached runs cannot judge the return signal, and round trip 1's `hidden`-with-no-`visible` after the settle is confounded twice: the session was locked (LogonUI in session 1 — `SetForegroundWindow` answers False on a locked desktop, which is what forces the min/restore dance in the first place) and Chrome's occlusion tracker reports everything hidden while the session is locked.
- STOP-if #1 ("`visible` never arrives after `resumed`") is **not triggered**: it arrived once, from the exact call the hub makes. Design kept; the `window` `focus` fallback stays in the pocket, not in the code. STOP-if #2 (TD Snap left on top) could not be judged under a lock. **Dad's morning, unlocked, no wires:** Songs mid-song → 💬 → TD Snap in front → Songs tile → the same song continues. If the kiosk comes up but the song stays silent, that is the `focus`-listener line; if TD Snap stays on top, that is spec §10.
- Not this feature, found here: **the tablet has no `wmic`** (`C:\Windows\System32\wbem\WMIC.exe` absent — 24H2 dropped it), so `start-hub.bat:74`'s kill-previous-kiosk is a silent no-op on this device: a second tile while a different app is open hands the url to the running kiosk (the 9/6 taskbar bug is back on the tablet, and was before this feature). Fix = the bat POSTs `/kiosk/close` (the hub's PowerShell `closeKioskWindows`, server.js:1341) before launching, and `tests/start-hub-bat.test.mjs`'s wmic law moves with it. Also: `query`/`qwinsta`/`cut`/`timeout /t` are unusable over the tablet's sshd (`ping -n` waits instead); muted media barely advances while the page is hidden, so t-based evidence is only valid attached-and-visible.
- Muted-media tell for later QA: Chrome suspends a muted, hidden element (0.6 s of progress in 24 s) — never read a parked page's `currentTime` as "still playing".

**T10 — landed and released (9/17).** Dad's unlocked morning round trip: **"All worked perfectly"** — both STOP-ifs from step 3 closed by use. Landed in order (era-core a7a312b, era-board 54fcb20, era-making-words 3c2578f, era-pencil da25179, era-hub 27376a4), 99/0. Then, at dad's "deploy out publicly, let's also sign on this", the plan's `v0.33.6 --patch` became **signed v0.34.0** (`release.sh v0.34.0 --skip-gate`).
- Run 1 died in the VM harness: legs A/B still waited 120 s for `#door`, which the shared bar renamed `#barDoor` (586feeb → hub 5e46d52, docs/tests only). Run 2: build **20260917.1751**, legs A+B 14/14, signed on the SimplySign rail, tag + GitHub Release ~18:50 UTC; installer sha `7d60eb6d…`, latest.json `0d38d0b7…`.
- **Leg C (post-publish SmartScreen) went 0/5 and release.sh said PULL THE RELEASE.** First time the leg ever ran in anger (written 9/7, never green); every red was the harness, proven one at a time before believing any of it: (1) `smartscreen-facts.ps1` returned early when nothing was downloaded yet, so test 1's SS_*/DEF_* facts were undefined; (2) `connectOverCDP` points Edge's downloads at a Playwright directory on the Linux box (`Browser.setDownloadBehavior allowAndName`) — the guest's Edge showed "<guid> — Couldn't download - Download error"; the leg now hands Edge its own Downloads folder back (`behavior: default`) before the click; (3) `assert.notMatch` does not exist (`doesNotMatch`); (4) the probe's `Shell.Application.ShellExecute` never returns while the App Reputation interstitial is up (the check is modal in the caller), so the script that was meant to click More info sat behind it for 15 min — now fired from a throwaway child powershell; (5) on 19041 the interstitial is an explorer.exe `ApplicationFrameWindow` with an EMPTY title whose content belongs to `CHXSmartScreen.exe` — no `smartscreen.exe` window at all — so the by-process/by-title scan reported `SMARTSCREEN=absent` with the dialog on screen; the probe now finds the heading text anywhere on the desktop and climbs to its window; (6) the expanded dialog is a grid: "Publisher:" and the name are two DataItems, so the label-with-value regex would have said `not-shown`.
- What the installer itself did, harness aside: host and guest curl of the release URL → 302 → 302 → 200, 47,602,816 bytes, sha == checksums.txt; Edge downloaded it without a Keep prompt; MOTW ZoneId=3 with HostUrl release-assets.githubusercontent.com; `Get-AuthenticodeSignature` Valid, subject = the certificate's CN (Open Source Developer + dad's name), Certum Code Signing 2021 CA, Certum Timestamp 2026, NotAfter 2027-09-05; SmartScreen on, Defender RTP on, 0 threats. The double-click shows **"Windows protected your PC"** (a fresh certificate has no reputation yet — expected, per the leg's own contract) → More info → `App: New-ERA-Setup.exe / Publisher: <the certificate's CN>` → Run anyway → Choose Components. **Decision: v0.34.0 stays published.** Reputation builds from downloads over weeks; the day the interstitial stops appearing, the leg's VERDICT line will say so.
- Devices: tablet 20260917.0938 → 1751 and i13 (home, hub running, was v0.33.5) → 1751 by `POST /update/check` over the standing tunnels (stale `ssh -f -N -L` tunnels answer nothing: kill the pid and re-open first). Settings intact on both (`exitTo/pauseGoes: tdsnap`). Live 8377 on the build box not restarted.
- Guest facts for next time: Edge cold start 2–5 min, the whole leg ~25 min; `vm.guest()` with quoted paths returns empty through the nested ssh; the worktree sandbox refuses most `a && b` chains that touch paths outside the worktree — one plain command per call, long jobs in scratchpad wrapper scripts under `setsid nohup`.
- Leg C tally against the published v0.34.0, worktree harness: run 2 (fixes 1–2) 3/5; run 3 (+3, +4) 3/5 — `SMARTSCREEN=absent` with the dialog on screen; run 4 (+5, +6) 4/5 — `PUBLISHER=":"` (the bare-label regex captured the colon); **run 5 (20:35 UTC) 5/5, rc 0**: `# VERDICT SmartScreen interstitial: SHOWN — publisher "<the certificate's CN>", UAC absent`, `INSTALLER=up`, Defender threats=0. release.sh's own leg C stays the gate for the next signed cut.
